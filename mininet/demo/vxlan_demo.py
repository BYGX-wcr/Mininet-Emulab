...
switch_image = "localhost/p4switch-frr:rockylinux-v1"
switch_dict = dict()
switch_count = 0
as_map = dict()
as_peers_map = dict()
...
hs_pairs = [("s0", "v0"), ("s1", "v1")]
for t in hs_pairs:
    sid = t[0]
    hid = t[1]

    ip2 = snet_list[10].allocateIPAddr()
    link = net.addLink(switch_dict[sid], host_dict[hid], ip2=ip2, addr1=Subnet.ipToMac(ip1), addr2=Subnet.ipToMac(ip2))
    snet_list[10].addNode(switch_dict[sid])

    switch_dict[sid].attachIntfToL2VNI(link.intf1.name, vni=10)

    print("gw {}".format(l2vni_gw[sid][10]))
    host_dict[hid].setDefaultRoute("gw {}".format(l2vni_gw[sid][10]))

    nodes.addLink(switch_dict[sid].name, host_dict[hid].name, l2vni_gw[sid][10], ip2)
...
for t in hs_pairs:
    ...
    host_dict[hid].setDefaultRoute("gw {}".format(l2vni_gw[sid][11]))

    nodes.addLink(switch_dict[sid].name, host_dict[hid].name, l2vni_gw[sid][11], ip2)
